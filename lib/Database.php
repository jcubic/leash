...
class SQLiteResource {
    private $res;
    private $version;
    function __construct($res, $version) {
        $this->res = $res;
        $this->version = $version;
    }
    function __call($name, $arguments) {
        return call_user_func_array(array($this->res, $name), $arguments);
    }
    function fetchAll() {
        if ($this->version == 2) {
            return $this->res->fetchAll(SQLITE_ASSOC);
...
class SQLite {
    ...
    function query($query) {
        if ($this->version == 2 || $this->version == 3) {
            return new SQLiteResource($this->db->query($query), $this->version);
        } else {
            throw new Exception('SQLite not installed');
        }
    }
